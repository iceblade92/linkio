...
func Test_requestLogger(t *testing.T) {
	...
	if expectedLogString == logBuffer.String() && expectedStatusCode == rr.Code {
		t.Errorf("bouth expectedLogString:%s and expectedStatusCode:%v did not match logs", expectedLogString, expectedStatusCode)
	}
}
